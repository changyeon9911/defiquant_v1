...
class UpbitStrategyManager(StrategyManager):
    ...
    def upto_balance(self, desired_ratio, commission_rate=0.001):
        ...
        userdata = json.load(self.backtest_manager.get_userdata().json())
...
#        return self.backtest_manager.interface.exchange_info(params_dict)
    ...
    def volume_momentum(self, params_dict : dict, window=10):
        ...
        def volume_momentum(candlestick_df : pd.DataFrame, window=window):
            investment_df = candlestick_df.copy()[["candle_date_time_utc", "open_price", "trade_price", "candle_acc_trade_volume"]]
            investment_df["Previous Volume"] = investment_df["candle_acc_trade_volume"].shift(1)
            investment_df["Long Ratio"] = (investment_df["Previous Volume"] - investment_df["Previous Volume"].rolling(window).mean())/investment_df["Previous Volume"].rolling(window).mean()
